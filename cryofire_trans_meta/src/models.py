...
class SharedCNN(nn.Module):
    ...
    def forward(self, y_real):
        ...
        """
        x = self.cnn(y_real[:, None])
        x = x.reshape(x.size(0),self.final_channels,-1).transpose(0,1).transpose(0,2)
        # x的形状[final_size*final_size, batch_size, final_channels]
        x = self.transformer_encoder(x)
        return x.view(-1, self.final_channels, self.final_size, self.final_size)
